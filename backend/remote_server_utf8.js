    ...
    try {
        ...
        const itemsHtml = (order.items || []).map(item => `
            <tr>
                ...
                <td style="padding: 10px; border-bottom: 1px solid #eee; text-align: right; font-weight: 600;">Γé╣${item.price}</td>
            </tr>
        `).join('');
        ...
        const mailOptions = {
            ...
            to: user.email,
            subject: `Hooray! Your order ${order.orderId} is confirmed ≡ƒî┐`,
            html: `
                <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; background-color: #ffffff; border: 1px solid #eef2f3; border-radius: 16px; overflow: hidden;">
                    <div style="background: linear-gradient(135deg, #2d8c6f, #1a5d1a); padding: 40px 20px; text-align: center; color: white;">
                        ...
                        <p style="margin: 10px 0 0; opacity: 0.9; font-size: 16px;">≡ƒî▒ Your Green Journey Just Got Better!</p>
                    </div>
                    <div style="padding: 30px;">
                        ...
                        <div style="background-color: #f9fafb; border-radius: 12px; padding: 20px; margin: 25px 0;">
                            ...
                            <div style="margin-top: 20px; text-align: right;">
                                ...
                                <p style="margin: 10px 0; color: #1a5d1a; font-size: 20px; font-weight: 800;">Total: Γé╣${order.totalAmount}</p>
                            </div>
                        </div>
...
const sendOrderStatusEmail = async (order) => {
    try {
        ...
        if (order.status === 'Processing') {
            ...
            statusText = 'Delivered & Green! ≡ƒÄü';
        ...
        const mailOptions = {
            from: `"Greenie Culture" <${process.env.GMAIL_USER || 'greenieculture202@gmail.com'}>`,
            to: userEmail,
            subject: `Update for Order ${order.orderId}: ${order.status} ≡ƒî┐`,
            html: `
                <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; background-color: #ffffff; border: 1px solid #eef2f3; border-radius: 16px; overflow: hidden;">
...
mongoose.connect(MONGODB_URI, {
    ...
    .then(async () => {
        ...
        console.log('≡ƒôè Active Database:', mongoose.connection.name);

        // Run seeds only after connection is established
        console.log('≡ƒöä Starting data seeding...');
        try {
            await Promise.all([seedPlacements(), seedFaqs(), seedCouriers()]);
            console.log('Γ£à Seeding completed');
        } catch (seedErr) {
            console.error('ΓÜá∩╕Å Seeding internal error:', seedErr.message);
        }

        // Start listening only after DB is ready (or skip if port already in use)
        app.listen(PORT, '127.0.0.1', () => {
            console.log(`≡ƒÜÇ Server is running on port ${PORT} at http://127.0.0.1:${PORT}`);
        });
    })
    .catch(err => {
        console.error('Γ¥î MongoDB connection error:', err);
        console.log('≡ƒÆí TIP: Check if your IP is whitelisted in MongoDB Atlas and if your connection string is correct.');
    });
...
const sendSMSWhatsAppNotification = async (order) => {
    try {
        ...
        // --- MESSAGE TEMPLATE ---
        const message = `≡ƒî┐ Greenie Culture: Order ${orderId} Update! 
Status: ${status}
...
app.post('/api/auth/admin-register', auth, async (req, res) => {
    try {
        ...
        // ΓöÇΓöÇ Verify current admin's password ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        // ΓöÇΓöÇ Register new admin ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
app.get('/api/products/search', async (req, res) => {
    try {
        ...
        console.log(`\n≡ƒöì [ADMIN SEARCH] Query: "${q}"`);
        console.log(`≡ƒòÉ Time: ${new Date().toISOString()}`);
...
app.delete('/api/admin/products/:id', auth, async (req, res) => {
    try {
        console.log(`\n≡ƒùæ∩╕Å  [ADMIN DELETE] Attempting to delete product with ID: ${req.params.id}`);
        const deletedProduct = await Product.findByIdAndDelete(req.params.id);

        if (!deletedProduct) {
            console.log(`Γ¥î [ADMIN DELETE] Product not found for ID: ${req.params.id}`);
            return res.status(404).json({ message: 'Product not found' });
        }

        console.log(`Γ£à [ADMIN DELETE] Successfully deleted product: ${deletedProduct.name}`);
        ...
        if (count === 0) {
            const defaultFaqs = [
                ...
                },
                {
                    ...
                    answer: 'We deliver within 3ΓÇô7 business days depending on your location. Metro cities usually receive orders within 2ΓÇô3 days.',
                    order: 3
                },
                {
                    category: 'Plants',
