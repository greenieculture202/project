...
import { Component, inject, ChangeDetectorRef, NgZone, HostListener } from '@angular/core';
...
let AdminPanelComponent = class AdminPanelComponent {
    constructor() {
        this.searchTerm = '';
        this.activeMainCategory = 'Plants';
        this.activeCompanionGroup = 'All';
        ...
        this.notiService = inject(NotificationService);
        ...
        this.faqCategories = ['Orders', 'Payment', 'Delivery', 'Plants', 'Returns', 'Account'];
        this.userFilter = 'All';
        this.users = [];
        this.orders = [];
        ...
        this.hoveredPaymentSegment = null;
        this.isLoadingOrders = false;
        this.backendRevenue = 0;
        // Admin Payment View State
        this.adminPaymentFilterMethod = 'All';
        ...
        this.currentAdminSettings = {
            fullName: '',
            oldPassword: '',
            newPassword: ''
        };
        this.newAdminPayload = {
            fullName: '',
            email: '',
            password: '',
        ...
        this.aboutSectionTypes = ['journey', 'value', 'stat', 'quote', 'founder'];
        ...
        this.currentReplyCourierName = '';
        this.courierReplyMessage = '';
        this.inquiryActiveSubTab = 'customers';
        this.showAddProductToOfferModal = false;
        ...
        this.pieChartStyle = '';
        this.salesPieChartStyle = '';
        this.lineChartPath = '';
        this.areaChartPath = '';
        // Detailed Interactive Sales Chart
        this.salesSlices = [];
        this.daySlices = [];
        this.hoveredSalesSegment = null;
        this.totalSalesItems = 0;
        this.totalWeeklyOrders = 0;
        this.newUsersThisWeek = 0;
        this.newUsersPercentage = 0;
        // Notification State
        ...
        this.notificationAudio = new Audio('https://www.soundjay.com/buttons/sounds/button-3.mp3');
        this.adminNotifications = [];
        this.isFirstNotiLoad = true;
        this.categoryStats = [
    ...
    onSearchChange(immediate = false) {
    ...
    downloadInvoice() {
        if (this.isDownloading)
            return;
        const invoiceElement = document.querySelector('.printable-invoice-wrap');
        if (!invoiceElement) {
    ...
    setTab(tab, mainCategory, updateUrl = true) {
        this.activeTab = tab;
        if (mainCategory) {
    ...
    updateAdminProfile(event) {
        ...
        const payload = { fullName: this.currentAdminSettings.fullName };
    ...
    loadAdminNotifications() {
        ...
        this.http.get('/api/admin/notifications', { headers }).subscribe({
            next: (data) => {
    ...
    openCourierReply(courierName, event) {
        event.stopPropagation();
    ...
    sendCourierReply() {
        if (!this.courierReplyMessage.trim())
            return;
        const payload = {
    ...
    openEditOfferModal(offer) {
        ...
        this.newOffer = { ...offer };
        if (!this.newOffer.features)
            this.newOffer.features = ['', '', ''];
        this.showAddOfferModal = true;
        this.toggleBodyScroll(true);
    }
    closeAddOfferModal() {
    ...
    submitOffer() {
        ...
        offerData.features = offerData.features.filter((f) => f && f.trim() !== '');
    ...
    deleteOfferAdmin(id, title, image) {
    ...
    getCategoryList() {
        if (!this.mainCategoryGroups[this.activeMainCategory])
            return [];
        ...
        if (this.activeMainCategory === 'Accessories') {
            ...
            if (this.activeCompanionGroup === 'All')
                return allHeaders;
            if (this.activeCompanionGroup === 'Accessories')
                return accHeaders;
            if (this.activeCompanionGroup === 'Soil & Growing Media')
                return soilHeaders;
            if (this.activeCompanionGroup === 'Fertilizers & Nutrients')
                return fertHeaders;
            if (this.activeCompanionGroup === 'Gardening Tools')
                return toolHeaders;
        }
        return cats;
    }
    getProductsForCategory(category) {
        ...
        const itemsToSearch = [category.toLowerCase(), ...navItems.map((i) => i.toLowerCase())];
        ...
        const filtered = uniqueProducts.filter((p) => {
            ...
            return itemsToSearch.some(item => pName.includes(item) || pCat === item || pTags.includes(item));
        });
    ...
    getFilteredCategoriesForModal() {
        ...
        if (!this.isEditingProduct && this.newProduct.tags) {
            ...
            if (offerCode === 'G-INDOOR-6-SEC') {
                return categories.filter(c => c.key === 'Indoor Plants' || c.key.includes('Indoor'));
            }
            if (offerCode === 'G-GARDEN-6-SEC') {
                return categories.filter(c => c.key === 'Gardening Tools' || c.key.includes('Tool'));
            }
            if (offerCode === 'G-FLOWER-6-SEC') {
                return categories.filter(c => c.key === 'Flowering Plants' || c.key.includes('Flower'));
            }
            if (offerCode === 'G-BOGO-6-SECTION') {
                return categories.filter(c => c.key === 'XL Plants' || c.key.includes('XL'));
            }
        }
        // Original logic for grouping by activeMainCategory and activeCompanionGroup
        if (!this.activeMainCategory || !this.mainCategoryGroups[this.activeMainCategory])
            return [];
        if (this.activeMainCategory !== 'Accessories') {
        ...
        targetSubGroups.forEach((group) => {
            (this.navbarDefinitions[group] || []).forEach((item) => validCategories.add(item));
        });
        return this.mainCategoryGroups['Accessories'].filter(cat => validCategories.has(cat.key));
    }
    getTypeBadgeClass(count) {
        if (count > 10)
            return 'high-count';
        if (count > 0)
            return 'active-count';
    ...
    get filteredProductCategories() {
        if (this.activeMainCategory === 'Accessories') {
            if (this.productCategoryFilter === 'All') {
                ...
                return this.getCategoryList().map((c) => c.key);
            }
        ...
        const categories = this.getCategoryList().map((c) => c.key);
        if (this.productCategoryFilter === 'All')
            return categories;
        return [this.productCategoryFilter];
    }
    ...
    openAddProductModal(initialTag) {
        this.isEditingProduct = false;
        this.editingProductId = null;
    ...
    submitProduct() {
        ...
        if (!this.isEditingProduct) {
            ...
            for (const cat in this.productMap) {
                if (Array.isArray(this.productMap[cat])) {
                    const found = this.productMap[cat].find((p) => p.name.toLowerCase().trim() === this.newProduct.name.toLowerCase().trim());
                    if (found) {
                        existingProd = found;
                        break;
                    }
        ...
        const cleanedImages = this.newProduct.images.filter((img) => img && img.trim() !== '');
    ...
    triggerSuccessModal(name, category, type, image) {
        this.successModalData = {
            name: name,
    ...
    get filteredFaqCategories() {
        if (this.faqCategoryFilter === 'All')
            return this.faqCategories;
        return [this.faqCategoryFilter];
    }
    getFaqsForCategory(category) {
    ...
    get filteredOrdersV2() {
        ...
        if (this.orderSearchQuery && this.orderSearchQuery.trim() !== '') {
            ...
            filtered = filtered.filter(o => o.orderId.toLowerCase().includes(query) ||
    ...
    updateOrderStatus(orderId, newStatus) {
        ...
        if (currentCourier)
            body.courierName = currentCourier;
        if (this.trackingNumber)
            body.trackingNumber = this.trackingNumber;
        if (this.expectedDeliveryDate)
            body.expectedDeliveryDate = this.expectedDeliveryDate;
    ...
    sendWhatsAppUpdate(order) {
        ...
        const message = encodeURIComponent(`*Greenie Culture Order Update*\n\n` +
    ...
    isTrackingDisabled(orderId) {
        const state = this.trackingGenLimits[orderId];
        if (!state)
            return false;
        return state.count >= 1 && Date.now() < state.lockUntil;
    }
    createTrackingNumberString() {
        const prefix = 'GC';
    ...
    deleteAboutAdmin(id, title) {
        this.deleteTarget = { id, name: title, type: 'about' };
        this.deleteSuccess = false;
        this.showDeleteModal = true;
        this.toggleBodyScroll(true);
    }
    getOfferCodeByLink(ctaLink) {
        if (!ctaLink)
            return null;
    ...
    removeProductFromOffer(product) {
        if (!confirm(`Are you sure you want to remove ${product.name} from this offer?`))
            return;
        // Clone the product and remove the activeOfferCode from tags
        const updatedProduct = { ...product };
        if (Array.isArray(updatedProduct.tags)) {
            updatedProduct.tags = updatedProduct.tags.filter((t) => t !== this.activeOfferCode);
        }
    ...
    get filteredProductsForOfferList() {
        if (!this.addProductSearchTerm.trim())
            return this.allProductsList.slice(0, 50); // Limit initial view
        const term = this.addProductSearchTerm.toLowerCase();
        return this.allProductsList.filter(p => p.name.toLowerCase().includes(term) ||
            (p.category || '').toLowerCase().includes(term)).slice(0, 50);
    }
    ...
    submitReply() {
        if (!this.selectedInquiry || !this.replyContent.trim())
            return;
        this.isSubmitting = true;
    ...
    updateDashboardStats() {
        ...
        if (this.orders && this.orders.length > 0) {
            ...
            if (latestOrderDate) {
                const latestOrder = latestOrderDate;
    ...
    loadCouriers() {
        const token = sessionStorage.getItem('auth_token');
        this.http.get('/api/admin/couriers', { headers: { 'x-auth-token': token || '' } }).subscribe({
            next: (data) => {
                this.couriers = data;
                this.courierStateMapping = {};
                this.courierFees = {};
    ...
    toggleCourierSettled(order) {
        const token = sessionStorage.getItem('auth_token');
        const newStatus = !order.courierSettled;
        this.http.put(`/api/admin/orders/${order._id}/courier-settled`, { courierSettled: newStatus }, { headers: { 'x-auth-token': token || '' } }).subscribe({
            next: (updated) => {
    ...
    getGlobalSettlement() {
        ...
        return this.couriers.map(c => {
            const courierOrders = this.orders.filter(o => o.courierName === c.name || this.extractState(o) === this.getCourierForState(this.extractState(o)));
...
], AdminPanelComponent.prototype, "onDocumentClick", null);
