...
let AiAnalyticsPanelComponent = class AiAnalyticsPanelComponent {
    constructor() {
        ...
        this.isLoading = true;
        // Marketing & Growth
        this.marketingContent = signal(null);
        this.lastType = 'social';
        this.isMarketingLoading = signal(false);
        this.marketingProductName = signal('');
        this.marketingOfferDetails = signal('');
        // Operations Hub
        this.aiTasks = signal([]);
        this.isTasksLoading = signal(false);
        this.plantReminders = signal([]);
        this.groupedReminders = signal([]);
        this.selectedUserGroup = signal(null);
        this.showRemindersModal = signal(false);
        this.isRemindersLoading = signal(false);
        // Universal Search
        this.searchQuery = '';
        this.isSearchVisible = signal(false);
        this.searchResults = signal([]);
        // Neural Insights
        this.insightStream = signal([]);
        this.possibleInsights = [
        ...
        this.courierUsage = [];
        this.mostUsedCourier = '';
        this.stateWithHighestOrders = '';
        this.totalRevenue = 0;
        ...
        this.statePaths = INDIA_STATE_PATHS;
        ...
        this.userMessage = '';
        this.isChatLoading = signal(false);
        this.regionalIntelligence = signal({});
        // Category Filtering
        this.allCategories = signal(['All', 'Indoor', 'Outdoor', 'Flowering', 'Gardening', 'Seeds', 'Accessories', 'Tools', 'XL Plants']);
        this.selectedCategory = signal('All');
        this.allProducts = [];
        this.filteredInventory = signal([]);
    }
    ngOnInit() {
    ...
    loadHistory() {
        ...
        if (!token)
            return;
        this.http.get('/api/admin/chat-history', {
    ...
    getStateColor(stateId) {
        ...
        if (count === 0)
            return 'rgba(20, 184, 166, 0.1)'; // Very faded teal/green
        if (count < 3)
            return 'rgba(16, 185, 129, 0.4)'; // Emerald
        if (count < 8)
            return 'rgba(16, 185, 129, 0.6)';
        if (count < 15)
            return 'rgba(5, 150, 105, 0.8)'; // Deep Emerald
        return 'rgba(6, 95, 70, 1)'; // Darkest Forest
    }
    getStateOrderCount(stateId) {
        return this.stateOrderCounts[stateId] || 0;
    }
    getCourierColor(percent) {
        if (percent > 60)
            return 'linear-gradient(90deg, #10b981, #059669)'; // Emerald
        if (percent > 30)
            return 'linear-gradient(90deg, #84cc16, #65a30d)'; // Lime
        return 'linear-gradient(90deg, #f59e0b, #d97706)'; // Stay orange for warning
    }
    getProjectionBarHeight(index) {
        if (!this.projectionValues.length)
            return 0;
    ...
    fetchData() {
        ...
        this.http.get('/api/admin/orders', { headers }).subscribe({
            next: (data) => {
                this.orders = data;
    ...
    fetchRegionalStats() {
        ...
        if (!token)
            return;
        this.http.get('/api/admin/regional-analytics', {
            headers: { 'x-auth-token': token }
        }).subscribe({
    ...
    fetchProducts(headers) {
        this.http.get('/api/products', { headers }).subscribe({
            next: (products) => {
    ...
    generateAIResponse(query, retryCount = 0) {
        const token = sessionStorage.getItem('auth_token');
        const headers = { 'x-auth-token': token || '' };
        this.http.post('/api/admin/ai-assistant', {
            message: query,
            contextData: {
    ...
    // Growth Hub Methods
    generateMarketing(type, prod, details) {
        ...
        const token = sessionStorage.getItem('auth_token');
        this.http.post('/api/admin/generate-marketing', { type, productName: prod, offerDetails: details }, {
            headers: { 'x-auth-token': token || '' }
        }).subscribe({
            next: (res) => {
    ...
    getAiTasks() {
        ...
        const token = sessionStorage.getItem('auth_token');
        this.http.get('/api/admin/ai-tasks', {
            headers: { 'x-auth-token': token || '' }
        }).subscribe({
            next: (res) => {
    ...
    getPlantReminders() {
        ...
        const token = sessionStorage.getItem('auth_token');
        this.http.get('/api/admin/plant-reminders', {
            headers: { 'x-auth-token': token || '' }
        }).subscribe({
            next: (res) => {
...
], AiAnalyticsPanelComponent.prototype, "chatScrollContainer", void 0);
AiAnalyticsPanelComponent = __decorate([
    Component({
...
], AiAnalyticsPanelComponent);
export { AiAnalyticsPanelComponent };
//# sourceMappingURL=ai-analytics-panel.js.map