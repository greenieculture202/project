...
let CategoryNavComponent = class CategoryNavComponent {
    constructor() {
    ...
    getMenuColumns(cat) {
        ...
        const columns = [];
        if (cat === 'Seeds') {
            if (menu.vegetableSeeds)
                columns.push({ title: 'Vegetable Seeds', items: menu.vegetableSeeds });
            if (menu.fruitSeeds)
                columns.push({ title: 'Fruit Seeds', items: menu.fruitSeeds });
            if (menu.herbSeeds)
                columns.push({ title: 'Herb Seeds', items: menu.herbSeeds });
            if (menu.flowerSeeds)
                columns.push({ title: 'Flower Seeds', items: menu.flowerSeeds });
            if (menu.microgreenSeeds)
                columns.push({ title: 'Microgreen Seeds', items: menu.microgreenSeeds });
        }
        else if (cat === 'Soil & Growing Media') {
            if (menu.soilTypes)
                columns.push({ title: 'Soil Types', items: menu.soilTypes });
            if (menu.organicAmendments)
                columns.push({ title: 'Organic Amendments', items: menu.organicAmendments });
            if (menu.growthMedia)
                columns.push({ title: 'Growth Media', items: menu.growthMedia });
        }
        else if (cat === 'Fertilizers & Nutrients') {
            if (menu.organicFertilizers)
                columns.push({ title: 'Organic Fertilizers', items: menu.organicFertilizers });
            if (menu.chemicalFertilizers)
                columns.push({ title: 'Chemical Fertilizers', items: menu.chemicalFertilizers });
            if (menu.plantBoosters)
                columns.push({ title: 'Plant Boosters', items: menu.plantBoosters });
        }
        else if (cat === 'Gardening Tools') {
            if (menu.handTools)
                columns.push({ title: 'Hand Tools', items: menu.handTools });
            if (menu.cuttingTools)
                columns.push({ title: 'Cutting Tools', items: menu.cuttingTools });
            if (menu.diggingTools)
                columns.push({ title: 'Digging Tools', items: menu.diggingTools });
            if (menu.powerTools)
                columns.push({ title: 'Power Tools', items: menu.powerTools });
        }
        else if (cat === 'Accessories') {
            if (menu.potsAndPlanters)
                columns.push({ title: 'Pots & Planters', items: menu.potsAndPlanters });
            if (menu.wateringEquipment)
                columns.push({ title: 'Watering Equipment', items: menu.wateringEquipment });
            if (menu.plantSupport)
                columns.push({ title: 'Support & Protection', items: menu.plantSupport });
            if (menu.lightingEquipment)
                columns.push({ title: 'Lighting Equipment', items: menu.lightingEquipment });
            if (menu.decorativeAccessories)
                columns.push({ title: 'Decorative & Display', items: menu.decorativeAccessories });
        }
        else if (cat === 'Gardening') {
            if (menu.outdoorBlooms)
                columns.push({ title: 'Outdoor Blooms', items: menu.outdoorBlooms });
            if (menu.homeGardenEssentials)
                columns.push({ title: 'Home Garden Essentials', items: menu.homeGardenEssentials });
            if (menu.foliageAndGreens)
                columns.push({ title: 'Foliage & Greens', items: menu.foliageAndGreens });
            if (menu.decorativeStyle)
                columns.push({ title: 'Decorative Style', items: menu.decorativeStyle });
        }
        else {
            if (menu.eliteGreens)
                columns.push({ title: 'Elite Greens', items: menu.eliteGreens });
            if (menu.perfectPlacements)
                columns.push({ title: 'Perfect Placements', items: menu.perfectPlacements });
            if (menu.plantsCollection)
                columns.push({ title: 'Collection', items: menu.plantsCollection });
            if (menu.plantersStyle)
                columns.push({ title: 'Style', items: menu.plantersStyle });
            if (menu.sendTo || menu.sendPlantsTo)
                columns.push({ title: 'Delivery To', items: menu.sendTo || menu.sendPlantsTo });
        }
        return columns;
    }
    showMenu(category) {
    ...
    navigateToItem(category, item, event) {
        // Just hide the menu so the user can see the navigation happened
        this.hideMenu();
    }
    hasMenu(category) {
    ...
    getItemLink(category, item, columnTitle) {
...
export { CategoryNavComponent };
//# sourceMappingURL=category-nav.js.map