...
let CheckoutComponent = class CheckoutComponent {
    constructor() {
        ...
        this.selectedOptionId = signal('standard');
        ...
        this.itemsTotal = computed(() => {
            return this.items().reduce((acc, item) => acc + (item.price * item.quantity), 0);
        });
        this.deliveryCharge = computed(() => {
            const state = this.selectedState();
            if (!state || this.couriersList().length === 0)
                return 0;
            // Find if any courier serves this state
            const servingCouriers = this.couriersList().filter((c) => (c.states || []).map((s) => s.toLowerCase()).includes(state.toLowerCase()));
            const baseCharge = servingCouriers.length === 0 ? 0 : Math.min(...servingCouriers.map((c) => c.fee || 0));
        ...
        this.totalSavings = this.cartService.totalSavings;
        this.contactEmail = '';
        this.firstName = '';
        this.lastName = '';
        this.address = '';
        ...
        this.isPaymentTimerActive = false;
            ...
            this.userService.getUserProfile().subscribe({
                next: (profile) => {
                    ...
                    if (!this.alternatePhone)
                        this.alternatePhone = profile.alternatePhone || '';
        ...
        if (savedState) {
            try {
                ...
                if (state.selectedOptionId)
                    this.selectedOptionId.set(state.selectedOptionId);
    ...
    downloadInvoice() {
        const invoiceElement = document.querySelector('.luxury-bill-card');
        if (!invoiceElement)
            return;
        // Temporarily hide the actions button row so it doesn't appear in the PDF
        const actionsElement = document.querySelector('.luxury-actions');
        if (actionsElement)
