...
let DeliveryPanelComponent = class DeliveryPanelComponent {
    constructor() {
        ...
        this.allOrders = [];
        this.comparisonData = [];
        this.topCourier = null;
        this.activeMainTab = 'orders';
        this.filterStatus = 'All';
        this.selectedCourier = '';
        this.shipCourier = '';
        this.searchQuery = '';
        this.activeCourierSubTab = 1; // 1: Orders Table, 2: Payment Summary, 3: Analytics
        this.isLoading = true;
        this.today = new Date();
        ...
        this.activeUnlockedCourier = null;
        ...
        this.activeSettlementCourier = '';
        this.selectedOrderIds = [];
        this.adminMessageText = '';
        this.courierNotifications = [];
    ...
    setTab(tab, courier, navigate = true) {
        ...
        if (navigate) {
            const queryParams = { tab: this.activeMainTab };
    ...
    selectSettlementCourier(courierName) {
    ...
    saveCourier() {
        ...
        const payload = {
            ...this.courierForm,
            states: this.courierForm.states.split(',').map((s) => s.trim()).filter((s) => s.length > 0)
        };
    ...
    generateCourierChart(courierName) {
        if (!courierName)
            return;
    ...
    get dashboardPendingOrders() {
        if (!this.dashboardChartCourier)
            return [];
        const states = this.courierStateMapping[this.dashboardChartCourier] || [];
        return this.allOrders.filter(o => {
            const orderState = this.extractState(o);
    ...
    getCourierCount(courier, status) {
    ...
    updateExpectedDate(orderId, event) {
    ...
    getCourierSettlement(courierName) {
        const states = this.courierStateMapping[courierName] || [];
    ...
    getSettlementForCourier(courierName) {
    ...
    transferAllPendingToAdmin(courierName) {
        const s = this.getCourierSettlement(courierName);
    ...
    sendAdminMessage(courierName, customMsg) {
        ...
        this.http.post('/api/admin/notify-courier', payload, {
            ...
            next: (res) => {
                if (!customMsg) {
                    this.notiService.show(isAdmin ? `Message successfully sent to ${courierName}` : 'Your message has been sent to the Admin', 'Message Sent', 'success', 'toast');
                    this.adminMessageText = '';
                }
    ...
    loadCourierNotifications(courierName) {
        this.http.get(`/api/courier/notifications/${courierName}`).subscribe({
            next: (notis) => {
    ...
    clearChatHistory(courierName) {
        if (!confirm('Are you sure you want to clear the entire chat history with the Admin?'))
            return;
        const token = sessionStorage.getItem('auth_token');
    ...
    transferCodToAdmin(courierName) {
        const s = this.getCourierSettlement(courierName);
        ...
        this.pendingSettlementData = {
            ...
            amount,
            orderCount,
            orderIds: s.codSettledOrders.map((o) => o._id)
        };
        this.showSettlementConfirmModal = true;
    }
    loadSettlementHistory(courierName) {
        const token = sessionStorage.getItem('auth_token');
        this.http.get(`/api/admin/settlements/${courierName}`, {
            headers: { 'x-auth-token': token || '' }
        }).subscribe({
            next: (history) => {
    ...
    autoCalculateExpectedDates() {
        ...
        this.allOrders.forEach(order => {
            ...
            if (!order.expectedDeliveryDate && order.status !== 'Delivered' && order.status !== 'Cancelled') {
                ...
                let daysToAdd = 0;
                if (type.includes('2 day'))
                    daysToAdd = 2;
                else if (type.includes('4 day'))
                    daysToAdd = 4;
                else if (type.includes('7 day'))
                    daysToAdd = 7;
                else if (type.includes('10 day'))
                    daysToAdd = 10;
                if (daysToAdd > 0) {
...
DeliveryPanelComponent = __decorate([
    Component({
...
export { DeliveryPanelComponent };
//# sourceMappingURL=delivery-panel.js.map