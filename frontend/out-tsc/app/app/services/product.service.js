...
let ProductService = class ProductService {
    ...
    getProducts(category, state, limit) {
        ...
        return this.http.get(url).pipe(timeout(10000), // Enforce 10 seconds timeout so spinner clears
        map(products => {
    ...
    getAllProductsMap() {
        return this.http.get(`${this.apiUrl}/products/map`).pipe(map(productMap => {
    ...
    getProductBySlug(slug) {
        ...
        return this.http.get(`${this.apiUrl}/products/slug/${slug}`).pipe(map(product => {
            ...
            for (const category in this.fallbackProducts) {
                const found = this.fallbackProducts[category].find(p => (p.slug === slug) || (this.convertToSlug(p.name) === slug));
                if (found) {
    ...
    getCategories() {
        ...
        return this.http.get(`${this.apiUrl}/products/categories`).pipe(map(categories => {
    ...
    searchProducts(term) {
        if (!term || term.length < 2)
            return of([]);
        return this.http.get(`${this.apiUrl}/products/search?q=${encodeURIComponent(term)}`).pipe(map(products => products.map(p => {
    ...
    getRelatedProducts(category, limit = 4) {
        if (!category) {
            return of([]);
        }
        ...
        return this.http.get(`${this.apiUrl}/products?category=${encodeURIComponent(category)}&limit=${limit}`).pipe(map(products => {
            products.forEach(p => {
                const slug = p.slug || this.convertToSlug(p.name);
                this.productCache.set(slug, p);
            });
    ...
    addProduct(product) {
        return this.http.post(`${this.apiUrl}/admin/products`, product, { headers: this.getHeaders() }).pipe(map(newProd => {
    ...
    deleteProduct(id) {
        return this.http.delete(`${this.apiUrl}/admin/products/${id}`, { headers: this.getHeaders() }).pipe(map(res => {
            this.categoryCache.clear();
            return res;
        }));
    }
    ...
    updateProduct(id, product) {
        return this.http.put(`${this.apiUrl}/admin/products/${id}`, product, { headers: this.getHeaders() }).pipe(map(updatedProd => {
            this.categoryCache.clear();
    ...
    getProductsByOfferCode(code) {
        return this.http.get(`${this.apiUrl}/products?category=${encodeURIComponent(code)}`).pipe(catchError(error => {
